...
async function GET() {
    try {
        ...
        console.error('Erro ao ler partidas:', error.message);
        return new Response(JSON.stringify({
            error: 'Erro ao ler partidas'
        }), {
            status: 500
        });
    }
...
async function POST(request) {
    try {
        const newPartida = await request.json();
        const data = __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].readFileSync(jsonPath, 'utf-8');
        const partidas = JSON.parse(data);
        const newId = partidas.length > 0 ? (Math.max(...partidas.map((p)=>Number(p.id))) + 1).toString() : '1';
        newPartida.id = newId;
        ...
        return new Response(JSON.stringify(newPartida), {
            status: 201
        });
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5e0bdb88._.js.map